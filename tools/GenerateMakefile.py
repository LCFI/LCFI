...
def initializeMakefile(sourceFiles):
  with open(fname, 'w') as fout:
    ...
    #fout.write('PRIMARY_HEADERS = Geometry.hpp SparseMatrix.hpp Vector.hpp CGData.hpp MGData.hpp hpcg.hpp' +'\n')
...
def constructMakeFile(sourceFiles):
  ...
  with open(fname, 'a') as fout:
    ...
    for codeFile in sourceFiles:
      objFile = re.sub(regex, fileextension, codeFile)
      if codeFile != "finalize.cpp" and codeFile !="init.cpp" and codeFile !="main.cpp":
        fout.write(objFile + ': ' + codeFile + " $(PRIMARY_HEADERS)")
        fout.write(indent)
