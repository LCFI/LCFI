...
static bool loop_s = false;
static bool loop = false;
static long long curr_loop=1;
static long long cur=1000000;
...
void _parseLLFIConfigFile() {
  ...
  while (fgets(line, CONFIG_LINE_LENGTH, ficonfigFile) != NULL) {
    ...
    if (strcmp(option, "fi_type") == 0) {
      ...
      config.fi_indexs[config.index_num]=config.fi_index;
      config.index_num++;
    ...
    //========================Update of L3FI=======================
      else if (strcmp(option, "loop_start") == 0) {
      config.loop_start = atoi(value);
      loop=true;
      assert(config.loop_start >= 0 && "invalid loop_start in config file");
    } else if (strcmp(option, "loop_number") == 0) {
      config.loop_number = atoi(value);
      loop=true;
      assert(config.loop_number >= 0 && "invalid loop_number in config file");
    } 
    //========================Update of L3FI=======================
...
bool preFunc(long llfi_index, unsigned opcode, unsigned my_reg_index, 
  ...
  if (config.fi_accordingto_cycle) {
    ...
    if (llfi_index == config.fi_index && !curr_cycle%10)
      inst_selected = true;
  }*/
  int i;
  ...
  if(!loop)
  {
    ...
    {
      if (llfi_index == config.fi_indexs[i])
      {
        curr_cycle+=1;
        //if (curr_cycle%10==0)
        inst_selected = true;
  ...
  {
    //fprintf(stderr, "llfi_index %ld cur %ld curr_loop %ld\n",llfi_index,cur,curr_loop);
    if(llfi_index == config.loop_start)
    {
      cur=0;
      if(curr_loop==config.loop_number)
      {
        loop_s=true;
      }
      else
    ...
    if(cur==1&&llfi_index!=config.loop_start+2)
    {
      curr_loop+=1;
    }

    if(loop_s && llfi_index == config.fi_index)
    {
      inst_selected=true;
    }
    cur+=1;
  ...
  //fprintf(stderr, "opcode%ld\n",opcode);
  //fprintf(stderr, "config.curr_cycle:%ld\n",curr_cycle);
  //fprintf(stderr, "config.opcodecyclearray[opcode]:%ld\n",opcodecyclearray[opcode]);
