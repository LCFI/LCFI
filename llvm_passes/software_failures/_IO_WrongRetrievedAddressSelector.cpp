...
//fidl_3
            ...
            funcNamesTargetArgs["fwrite"].insert(0);
...
//fidl_9
std::map<std::string, std::set<int> >  _IO_WrongRetrievedAddressInstSelector::funcNamesTargetArgs;

class _IO_WrongRetrievedAddressRegSelector: public SoftwareFIRegSelector {
    private:
...
//fidl_10
        if (_IO_WrongRetrievedAddressInstSelector::isTarget(CI, reg)) {
            return true;
        } else { 
            return false;
        }
...
static RegisterFIRegSelector B("WrongRetrievedAddress(IO)", new _IO_WrongRetrievedAddressRegSelector());
