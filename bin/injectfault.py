  ...
  if not os.path.isfile(fi_exe):
    ...
    for ii, run in enumerate(rOpt):
      ...
      if 'fi_type' in locals():
        del fi_type
      if 'loop_number' in locals():
        del loop_number
      if 'loop_start' in locals():
        del loop_start
      ...
      num_of_data=1
      if "num_of_data" in run["run"]:
        num_of_data=run["run"]["num_of_data"]
      ...
      ##================UPDATE of LCFI================================
      if "fi_index" in run["run"] and num_of_data == 1:
        fi_index=run["run"]["fi_index"][0]
        checkValues("fi_index",fi_index)
      if "loop_start" in run["run"]:
        loop_start=run["run"]["loop_start"]
        checkValues("loop_start",loop_start)
      if "loop_number" in run["run"]:
        loop_number=run["run"]["loop_number"]
        checkValues("loop_start",loop_number)
      if num_of_data >= 1:
        fi_index=run["run"]["fi_index"][0]
        for i in range(num_of_data):
          fi_indexs.append(run["run"]["fi_index"][i])
      ##================UPDATE of LCFI================================    
      ...
      for index in range(0, run_number):
        ...
        ##================UPDATE of LCFI================================
        #elif 'fi_index' in locals() and num_of_data == 1:
        #  ficonfig_File.write("fi_index="+str(fi_index)+'\n')
        
        elif 'fi_index' in locals():
          #print(num_of_data)
          for i in range(num_of_data):
            ficonfig_File.write("fi_index="+str(fi_indexs[i])+'\n')
        if 'loop_start' in locals():
          ficonfig_File.write("loop_start="+str(loop_start)+'\n')
        if 'loop_number' in locals():
          ficonfig_File.write("loop_number="+str(loop_number)+'\n')
